...
func (m *postgresDBRepo) InsertReservation(res models.Reservation) (int, error) {
	...
	err := m.DB.QueryRowContext(ctx, stmt,
		res.FirstName, res.LastName, res.Email, res.Phone, res.StartDate, res.EndDate, res.RoomID, time.Now(), time.Now(),
	).Scan(&newID)
...
// SearchAvailabilityByDates return trues if availability exists for roomID, and false if no availability exists
