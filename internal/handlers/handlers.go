...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	// converting the dates we get as strings into time objects
	...
	// good practice: any time you receive a POST request, direct the user to a new page with an HTTP redirect
...
// Majors renders the room page for the Major's Suite
...
func (m *Repository) PostAvailability(w http.ResponseWriter, r *http.Request) {
	...
	if len(rooms) == 0 {
		// this means there is no availability in any room
...
// Convention in Go is that if you have a type you only use in a few places: put it as close as possible to them
type jsonResponse struct {
	OK      bool   `json:"ok"`
	Message string `json:"message"`
}

// AvailabilityJSON handles requests for availability checks and sends JSON response
...
func (m *Repository) ReservationSummary(w http.ResponseWriter, r *http.Request) {
	...
	if reservation, ok := sessionValue.(models.Reservation); ok {
		...
		// Failed to retrieve a reservation
		m.App.ErrorLog.Printf("Error getting reservation from session. Value: %v, Type: %T\n", sessionValue, sessionValue)
